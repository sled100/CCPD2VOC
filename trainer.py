    ...
    def predict(self,
        ...
        for outs in results:
            ...
            for i, im_id in enumerate(outs['im_id']):
                ...
                frame_num=len(np.array(bbox_res))
                image_name_path,image_style=image_path.split('.')
                ...
                for dt in np.array(bbox_res):
                    ...
                    xmax=xmin+w
                    ymax=ymin+h
                    [frame_id,xmin,xmax,ymin,ymax]=[int(frame_id),int(xmin),int(xmax),int(ymin),int(ymax)]
                    frame_image=img_rgb[ymin:ymax,xmin:xmax]

                    frame_save_path=os.path.join(output_dir,image_name)
                    frame_save_name=image_name+'_'+'frame'+str(frame_id)+'.jpg'
                    frame_save_full_path=os.path.join(frame_save_path,frame_save_name)
                    frame_image_save_full_path=os.path.join(frame_save_path,image_name+'.jpg')

                    print('output_dir,image_name:',output_dir,image_name)
                    print('os.path.join(output_dir,image_name):',os.path.join(output_dir,image_name))
                    print('frame_save_path:',frame_save_path)
                    print('frame_image_save_full_path:',frame_image_save_full_path)
                    if not os.path.exists(frame_save_path):
                        os.makedirs(frame_save_path)
                    cv2.imwrite(frame_save_full_path,frame_image)
                    image.save(frame_image_save_full_path,quality=95)
